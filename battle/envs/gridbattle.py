...
class GridBattle:
    ...
    def __init__(self,
        ...
        self.shape: Tuple[int, int] = initial_map.shape  # grid shape (H_g, W_g)
    ...
    def _get_obs(self) -> Sequence[NDArray]:
        """
        Gets a full observation. This should only need ot be called once per step, it provides a copy of the whole grid
    ...
    def step(self, n_action: Sequence[NDArray]) -> Tuple[Sequence[NDArray], NDArray, bool, dict]:
        ...
        for rel_agent_id, (agent_action, agent) in enumerate(zip(n_action, self.agents)):
            ...
            normalized_actions[rel_agent_id] = np.broadcast_to(index_maps, (2, *agent_action.shape[-2:])).copy()
            ...
            # now, whether we were using logit maps, probability maps, or integers, it should be a map of coordinates
        ...
        for rel_agent_id in range(self.n_agents):
            ...
            for cell_axis0, cell_axis1 in zip(*np.where(self.grid == agent_id)):
                ...
                # print(normalized_actions[rel_agent_id].shape, cell_axis0, cell_axis1)
                # print(normalized_actions[rel_agent_id][cell_axis0, cell_axis1].shape)
                ...
                if self._valid_cell(attack_pos):
                    attacked_grid[rel_agent_id][tuple(attack_pos)] += 1  # indicate the tile is being attacked
        ...
        new_grid += converted_grid.astype(np.ubyte)
